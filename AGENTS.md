...
- **Keep the `tw-reuse` oneshot.** Without `tcp_tw_reuse=1` bindex's connection churn exhausts the container's ephemeral ports within a minute of indexing and electrs crash-loops; upstream only escapes it by talking to loopback. README § Image and Container Runtime.
